...
const BookmarkCard: React.FC<BookmarkCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    setFormattedDate(formatDate(bookmark.created_at));
  }, [bookmark.created_at]);

  const getSourceBadgeProps = (sourceType: BookmarkSourceType) => {
    switch (sourceType) {
      case 'raindrop':
        return { color: 'primary', label: 'Raindrop' };
      case 'manual':
        return { color: 'success', label: 'Manual' };
      case 'instapaper':
        return { color: 'dark', label: 'Instapaper' };
      case 'readwise':
        return { color: 'secondary', label: 'Readwise' };
      default:
        return { color: 'dark', label: sourceType };
    }
  };
      ...
      {hasImage ? (
        ...
        >
          <Group justify="space-between">
            ...
            {readTime && (
              <Text size="xs" style={{ color: 'var(--mantine-color-gray-6)' }}>{readTime} min read</Text>
            )}
          </Group>
        </Box>
      )}
      ...
      <Stack gap="sm" p="md" style={{ flex: 1, justifyContent: 'space-between' }}>
        <Stack gap="sm">
          ...
          {bookmark.description && (
            <Text 
              size="xs" 
              style={{ lineHeight: 1.5, color: 'var(--mantine-color-gray-6)' }}
              lineClamp={3}
            >
              {bookmark.description}
            </Text>
        ...
        <Group justify="space-between" mt="md">
          ...
          <Badge 
            size="sm"
            variant="light"
            color={(['primary', 'success', 'dark', 'secondary', 'accent'].includes(sourceProps.color) ? sourceProps.color : 'dark') as 'primary' | 'success' | 'dark' | 'secondary' | 'accent'}
          >
            {sourceProps.label}
          </Badge>
        </Group>
