...
| Maths teacher | School leadership, TeachFirst alumni, edtech | [docs/cvs/maths-teacher-cv.md](../cvs/maths-teacher-cv.md) | [`/teacher`](../../src/app/teacher/page.tsx) ✅ | **v2 — bespoke chalkboard design.** First persona off the shared Mantine template: own type stack (Bricolage/Lora/JetBrains Mono), full-bleed `teacher` surface with its own nav + footer, signature warm-cream + accent palette. Content unchanged (honest TeachFirst narrative; exam-result data gap still flagged) |
