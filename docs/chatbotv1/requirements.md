...
- **FR-CONV-2** — Multi-turn within a session: the bot remembers the previous N messages (target: full session, capped by token budget — see §7).
...
- **FR-KB-3** — Bundle is **static** and lives in the system prompt — no runtime retrieval or vector store in v1. Size budget: ≤ 8k tokens for the full bundle so it fits comfortably alongside the conversation context within Haiku's window. If the bundle outgrows the budget, the build step fails and forces a content-shrinking pass before v2 introduces retrieval.
...
- **FR-AGENT-6** — The model can emit a structured `draft_contact_message({ name?, email?, subject, body })` action. Field names match the existing form contract (`src/app/contact/page.tsx`: `name`, `email`, `subject`, `message` — `body` maps to `message`).
...
- **FR-RES-12** — Panel becomes an **anchored card**, bottom-right, with width `min(420px, calc(100vw - 32px))` and height `min(640px, calc(100dvh - 96px))`.
...
- **FR-RES-15** — From `--bp-lg` up the panel may grow to a max of 480×720 to take advantage of larger viewports, but never larger than that — this is a chat, not a workspace.
...
- **FR-VIS-3** — Patterns are Next.js-style route globs (e.g. `/projects/*`, `/projects/data-value-game/**`). Matching uses a small utility colocated in `src/lib/chat/` — no new dependency.
...
The branch is ready for the `dev` PR when **all** of the following are true:

1. Bubble renders on every public route, hidden on `/login` and `/auth/**`.
2. Opening the panel and sending "Who is Angus?" produces a streamed, grounded answer with at least one link to an internal page.
3. Asking "show me your habit tracker" produces a navigation button that, when clicked, routes to `/projects/habit`.
4. Ambiguous queries ("show me your AI work") produce ≥ 2 navigation options, not a guess.
5. The full injection test suite (≥ 20 prompts) passes in CI.
6. Rate limit triggers correctly under load test (21st message in 5 min is rejected).
7. Disabling JS still leaves the rest of the site usable (bubble simply does not appear).
8. Lighthouse score on `/` does not regress by more than 2 points on performance or accessibility.
9. Daily spend cap is configured and validated with a forced-trip integration test.
10. README or `docs/chatbotv1/README.md` documents how to run the chatbot locally and how to add new knowledge sources.
11. Manual responsive walkthrough passes on all five canonical sizes (375×667, 390×844, 768×1024, 1280×800, 2560×1440): launcher visible, panel opens correctly, composer stays above the on-screen keyboard on iOS Safari and Android Chrome, no horizontal scroll, no overlap with `/contact` submit button or `/work-with-me` CTAs.
12. `npm run check:breakpoints` passes — chat CSS modules contain zero raw `px` breakpoints (ADR-0032 §4 compliance).
13. Visibility config (`src/lib/chat/visibility.config.ts`) controls the launcher correctly: editing the file flips visibility on the target route without code changes elsewhere; deny-list pages (`/login`, `/auth/**`) do not render the bubble; unit test for allow/deny/forceShow precedence is green.
14. Contact pre-fill end-to-end: asking the bot "help me draft a message to Angus about X" produces a preview card, clicking the button routes to `/contact` with the draft populated (`subject`, `message`, and `name`/`email` if the user volunteered them), and the user can submit unchanged or edit first.
15. Persistence verified: a smoke test sends N messages and confirms `chat_sessions` + `chat_messages` rows are written (with hashed IP, never raw), and that the daily-spend aggregate query returns the expected total. RLS denies anonymous client reads.
16. Spend cap verified: setting `CHAT_DAILY_SPEND_CAP_USD` to a tiny value and exceeding it via the smoke test flips the bubble to the "resting" state and serves the static FAQ.
...
| Q6  | Where does the spend-cap config live                                                       | **Env var** `CHAT_DAILY_SPEND_CAP_USD`. Pricing inputs also via env (`CHAT_INPUT_PRICE_PER_MTOK`, `CHAT_OUTPUT_PRICE_PER_MTOK`). FR-RATE-4, TC-4. |
