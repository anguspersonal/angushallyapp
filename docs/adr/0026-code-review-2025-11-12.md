...
- **Detail:** `validatePassword` in `server/utils/authValidators.js` dereferences `password.length` without checking for null/undefined. When the password is missing, the auth route will throw a 500 instead of returning validation errors.
