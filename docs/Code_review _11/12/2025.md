...
- **Detail:** `src/routes/auth.js` only validates format and returns a success response without creating users, hashing passwords, or rate-limiting. Its behavior diverges from the main auth flow in `server/routes/authRoute.js`, risking accidental deployment of an insecure, non-functional endpoint if mounted.
- **Justification:** Prevents exposure of misleading or insecure auth endpoints; should either be removed or implemented with proper storage, hashing, and parity with the main auth system.
- **Priority:** Medium
- **Status:** Open
- **Date Created:** 2025-11-12
- **Last Updated:** 2025-11-12
