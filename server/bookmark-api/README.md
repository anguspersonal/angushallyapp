...
| 4 | **OmniSync** build canonical | Auto after sync / cron | `bookmarks.bookmarks`, `bookmarks.sync_log` | *Transfer logic pending implementation* |
...
4. **Transfer to canonical store** - Move to `bookmarks.bookmarks` with metadata enrichment ⏳ *Pending*
...
* A3 – Bookmark Transfer to Canonical
  * **Status**: **Not Implemented.** The canonical table (`bookmarks.bookmarks`) and related schemas exist in documentation (`docs/04_schema.md`) but no migration or application logic exists to transfer data from staging tables (`raindrop.bookmarks`).
  ...
  * **Missing:** Transfer logic between staging and canonical tables.
...
CREATE TABLE raindrop.bookmarks (
  id           SERIAL PRIMARY KEY,  -- Keep SERIAL for staging table
  user_id      UUID    NOT NULL REFERENCES identity.users(id),
  raindrop_id  VARCHAR NOT NULL,
  title        TEXT,
  link         TEXT,
  tags         TEXT[],
  is_organised BOOLEAN DEFAULT false,
  created_at   TIMESTAMPTZ DEFAULT now()
);
```
