...
exports.up = function(knex) {
  return knex.raw('CREATE EXTENSION IF NOT EXISTS pgcrypto')
    ...
    .then(async () => {
      const hasUsers = await knex.schema.withSchema('identity').hasTable('users');
      const hasRoles = await knex.schema.withSchema('identity').hasTable('roles');
      const hasUserRoles = await knex.schema.withSchema('identity').hasTable('user_roles');

      const promises = [];

      if (!hasUsers) {
        promises.push(
          knex.schema.withSchema('identity')
      ...
      const existingRoles = await knex('identity.roles').select('name');
      if (existingRoles.length === 0) {
        await knex('identity.roles').insert([
