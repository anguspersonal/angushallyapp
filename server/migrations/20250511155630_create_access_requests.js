...
exports.up = function(knex) {
  return knex.raw('CREATE SCHEMA IF NOT EXISTS identity')
    .then(async () => {
      const hasAccessRequests = await knex.schema.withSchema('identity').hasTable('access_requests');
      
      if (!hasAccessRequests) {
        await knex.schema.withSchema('identity')
        ...
        await knex.raw(`
          CREATE UNIQUE INDEX IF NOT EXISTS uq_access_requests_pending_user 
