...
/**
 * `/teacher` — Maths Teacher persona page (v2), client island.
 *
 ...
 *
 * Standalone shell (issue #129): in-page anchor nav to the persona's own
 * sections (Hero / About / Work / Contact), a per-persona light + dark palette
 * scoped under [data-surface="teacher"] (warm-paper day ↔ literal chalkboard
 * night) wired via PersonaThemeToggle, and a teacher-themed PersonaFooter whose
 * single discreet "angushally.com ↗" link is the only route back to the main
 * site — not first-class nav. The Contact section is a deliberate PLACEHOLDER:
 * the actual form is issue B2 (#132/#133/#134), built later.
 */
...
const TeacherClient = () => {
    return (
        <div className={`${styles.page} ${teacherFontClassNames}`} id="top">
            {/* ── TOP — brand + in-page anchor nav (the persona's OWN sections;
                no first-class full-site nav). Day/night lives here too. ── */}
            ...
            {/* ── ABOUT — what teaching taught me ── */}
            <section className={styles.lessons} id="about">
            ...
            {/* ── FOOTER — shared persona chrome, themed for teacher ──
                ...
                plus the ONE discreet "angushally.com ↗" link back to the main
                site (NOT first-class nav). Colours are driven by the
                --persona-* tokens set on `.footer` in teacher.module.css, so the
                neutral kit picks up the chalkboard palette in both modes. */}
            <PersonaFooter
